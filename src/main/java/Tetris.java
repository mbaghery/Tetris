...
public class Tetris extends Frame {
    ...
    TetrisCanvas myField = new TetrisCanvas(11, 20);
    ...
    public void paint(Graphics g) {
        ...
        if (myField.isCanvasFull()) paintGameOver(g);
    }
    ...
    private void paintBlock(Block b, Graphics g) {
        boolean[][] tempBlock = b.getPixels();
        for (int i = 0; i < tempBlock.length; i++) {
            for (int j = 0; j < tempBlock[i].length; j++) {
                if (!tempBlock[i][j].equals(Colors.WHITE)) paintBlockCell(tempBlock[i][j], i, j, g);
            }
        }
    }
    ...
    private void paintField(Canvas f, Graphics g) {
        Colors[][] tempField = f.getField();

        for (int i = 0; i < tempField.length; i++) {
            for (int j = 0; j < tempField[i].length; j++) {
                if (!tempField[i][j].equals(Colors.WHITE)) paintFieldCell(tempField[i][j], i, j, g);
            }
        }
    }

    private void paintBlockCell(boolean value, int x, int y, Graphics g) {
        ...
        public void keyPressed(KeyEvent ke) {
            switch (ke.getKeyCode()) {
                ...
                case KeyEvent.VK_SPACE:
                    ...
                    break;
                case KeyEvent.VK_F2:
                    myField = new TetrisCanvas(11, 20);
                    score = 0;
                    level = 1;
                    lines = 0;
                    t = new Timer();
